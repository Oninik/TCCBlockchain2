...
exports.registerVoter = async function (cpf, rg, nome) {
  ...
  if (!rg || !cpf || !nome) {
    let response = {};
    response.error = 'É preciso preencher todos os campos para se registrar!';
    return response;
  }
  ...
  try {
    ...
    if (userExists) {
      let response = {};
      console.log(`Uma identidade para o cpf ${cpf} já existe na carteira`);
      response.error = `Erro! uma identidade com o cpf ${cpf} já existe na carteira. Tente novamente com outro cpf`;
      return response;
    }
    ...
    const secret = await ca.register({ affiliation: 'org1', enrollmentID: cpf, role: 'client' }, adminIdentity);

    const enrollment = await ca.enroll({ enrollmentID: cpf, enrollmentSecret: secret });
    ...
    console.log(`Successfully registered voter ${nome}. Use cpf ${cpf} to login above.`);
    let response = `Successfully registered voter ${nome}. Use cpf ${cpf} to login above.`;
    return response;
  } catch (error) {
    console.error(`Failed to register user + ${cpf} + : ${error}`);
    let response = {};
    response.error = error;
    return response;
  }
